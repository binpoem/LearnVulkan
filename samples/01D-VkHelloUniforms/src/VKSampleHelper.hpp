...
struct QueueParameters
{
  ...
  QueueParameters() : Handle(VK_NULL_HANDLE), FamilyIndex(UINT32_MAX) {}
};
...
struct BufferParameters
{
  ...
  BufferParameters() : Handle(VK_NULL_HANDLE), Memory(VK_NULL_HANDLE), MappedMemory(nullptr), Descriptor(), Size(0) {}
};
...
struct SwapChainParameters
{
  ...
  SwapChainParameters()
    : Handle(VK_NULL_HANDLE), Format(VK_FORMAT_UNDEFINED), ColorSpace(VK_COLORSPACE_SRGB_NONLINEAR_KHR), Images(),
      Extent()
  {}
};
...
struct VulkanCommonParameters
{
  ...
  VulkanCommonParameters()
    : Instance(VK_NULL_HANDLE), PhysicalDevice(VK_NULL_HANDLE), Device(VK_NULL_HANDLE), GraphicsQueue(), ComputeQueue(),
      TransferQueue(), PresentationSurface(VK_NULL_HANDLE), SwapChain()
  {}
};
...
bool CheckPhysicalDeviceProperties(const VkPhysicalDevice &physicalDevice, VulkanCommonParameters &param);
