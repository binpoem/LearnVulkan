...
VKHelloUniforms::VKHelloUniforms(uint32_t width, uint32_t height, std::string name) : VKSample(width, height, name) {}
...
void VKHelloUniforms::OnUpdate()
{
  ...
  snprintf(m_lastFPS, (size_t)32, "%f fps", m_timer.GetFrameRate());
  m_frameCounter++;
...
void VKHelloUniforms::OnRender()
{
  ...
  VkResult acquire = vkAcquireNextImageKHR(m_vulkanParams.Device,
    ...
    UINT64_MAX,
    m_sampleParams.ImageAvailableSemaphore,
    nullptr,
    &imageIndex);
...
void VKHelloUniforms::PopulateCommandBuffer(uint32_t currentBufferIndex, uint32_t currentIndexImage)
{
  ...
  vkCmdBeginRenderPass(
    m_sampleParams.GraphicsCommandBuffers[currentBufferIndex], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
...
void VKHelloUniforms::SubmitCommandBuffer(uint32_t currentBufferIndex)
{
  ...
  submitInfo.pCommandBuffers =
    &m_sampleParams
       .GraphicsCommandBuffers[currentBufferIndex];// Command buffers(s) to execute in this batch (submission)
  submitInfo.commandBufferCount = 1;// One command buffer

  submitInfo.pWaitSemaphores =
    &m_sampleParams
       .ImageAvailableSemaphore;// Semaphore(s) to wait upon before the submitted command buffers start executing
  submitInfo.pSignalSemaphores =
    &m_sampleParams.RenderingFinishedSemaphore;// Semaphore(s) to be signaled when command buffers have completed
...
void VKHelloUniforms::CreateHostVisibleBuffer()
{
  ...
  VK_CHECK_RESULT(
    vkCreateBuffer(m_vulkanParams.Device, &bufferInfo, nullptr, &m_sampleParams.HostVisibleBuffer.Handle));
  ...
  memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits,
    ...
    m_deviceMemoryProperties);
  VK_CHECK_RESULT(
    vkAllocateMemory(m_vulkanParams.Device, &memAlloc, nullptr, &m_sampleParams.HostVisibleBuffer.Memory));
  ...
  VK_CHECK_RESULT(vkMapMemory(m_vulkanParams.Device,
    m_sampleParams.HostVisibleBuffer.Memory,
    0,
    memAlloc.allocationSize,
    0,
    &m_sampleParams.HostVisibleBuffer.MappedMemory));

  // Bind the buffer object to the backing host-visible device memory just allocated.
  VK_CHECK_RESULT(vkBindBufferMemory(
    m_vulkanParams.Device, m_sampleParams.HostVisibleBuffer.Handle, m_sampleParams.HostVisibleBuffer.Memory, 0));
...
void VKHelloUniforms::CreateDescriptorPool()
{
  ...
  VK_CHECK_RESULT(
    vkCreateDescriptorPool(m_vulkanParams.Device, &descriptorPoolInfo, nullptr, &m_sampleParams.DescriptorSet.Pool));
}
...
void VKHelloUniforms::CreateDescriptorSetLayout()
{
  ...
  VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
    m_vulkanParams.Device, &descriptorLayout, nullptr, &m_sampleParams.DescriptorSet.Layout));
}
...
void VKHelloUniforms::CreatePipelineLayout()
{
  ...
  VK_CHECK_RESULT(
    vkCreatePipelineLayout(m_vulkanParams.Device, &pPipelineLayoutCreateInfo, nullptr, &m_sampleParams.PipelineLayout));
}
...
void VKHelloUniforms::CreatePipelineObjects()
{
  ...
  shaderStages[0].module =
    LoadSPIRVShaderModule(m_vulkanParams.Device, GetAssetsPath() + "/data/shaders/triangle.vert.spv");
  ...
  shaderStages[1].module =
    LoadSPIRVShaderModule(m_vulkanParams.Device, GetAssetsPath() + "/data/shaders/triangle.frag.spv");
  // Main entry point for the shader
  ...
  VK_CHECK_RESULT(vkCreateGraphicsPipelines(
    m_vulkanParams.Device, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &m_sampleParams.GraphicsPipeline));
...
void VKHelloUniforms::UpdateHostVisibleBufferData()
{
  ...
  uBufVS.displacement[0] += translationSpeed * m_timer.GetDeltaTime();
  if (uBufVS.displacement[0] > offsetBounds) { uBufVS.displacement[0] = -offsetBounds; }
