...
VKHelloSCB::VKHelloSCB(uint32_t width, uint32_t height, std::string name) : VKSample(width, height, name) {}
...
void VKHelloSCB::PopulateCommandBuffer(uint32_t currentBufferIndex, uint32_t currentIndexImage)
{
  ...
  vkCmdBeginRenderPass(m_sampleParams.GraphicsCommandBuffers[currentBufferIndex],
    &renderPassBeginInfo,
    VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS);

  // Execute render commands from a secondary command buffer
  vkCmdExecuteCommands(
    m_sampleParams.GraphicsCommandBuffers[currentBufferIndex], 1, &m_secondaryCommandBuffers[currentIndexImage]);
...
void VKHelloSCB::AllocateSCBs()
{
  ...
  VK_CHECK_RESULT(
    vkAllocateCommandBuffers(m_vulkanParams.Device, &commandBufferAllocateInfo, m_secondaryCommandBuffers.data()));
}
...
void VKHelloSCB::PopulateSCBs()
{
  ...
  // Specifying the exact framebuffer that the SCBs will be executed with may result in better performance at command
